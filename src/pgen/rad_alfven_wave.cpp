...
void MHDEigensystem_linear_alfven(const Real d, const Real v1, const Real v2, const Real v3,
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    Real rem[7][7];
    Real ev[7];
    MHDEigensystem_linear_alfven(lwv.d0, lwv.v1_0, 0.0, 0.0, p0, lwv.b1_0, lwv.b2_0, lwv.b3_0,
    ...
    par_for("lin_alfven_mine1", DevExeSpace(), 0,nmb-1,ks,ke+1,js,je+1,is,ie+1,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      Real &x1min = size.d_view(m).x1min;
      Real &x1max = size.d_view(m).x1max;
      int nx1 = indcs.nx1;
  ...
  //Initial isotropic radiation energy in the domain
  auto &nh_c_ = pmbp->prad->nh_c;
  auto &tet_c_ = pmbp->prad->tet_c;
  auto &tetcov_c_ = pmbp->prad->tetcov_c;

  auto &i0 = pmbp->prad->i0;
  par_for("rad_alfven",DevExeSpace(),0,(pmbp->nmb_thispack-1),0,(n3-1),0,(n2-1),0,(n1-1),
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    for (int n=0; n<=nang1; ++n) {
      ...
      Real ii_tetrad = ii_coord;        //This should be the intensity in the tetad frame. This expression works for me 
                                        // because I am in Minkowski Frame. 
